...
class Proxy
{
    ...
    * ********************************************************************************************************************************* */
    public function toPhp8App(string $phpCommand) // : string
    {
        $output     = '';
        $returnCode = 0;
        $php8dir    = getenv('PHP8APP');
        $php8bin    = getenv('PHP8BIN');
...
COMMAND;
        ...
        if ($result === false || $returnCode > 0) {
            throw new Exception('Failed to execute command in PHP8: ' . $command . ' code: ' . $returnCode);
        }
        return $result;
    }
    ...
    {
        return '<?php require \'' . getenv('PHP8APP') . '/bootstrap.php\';' . $command . '?>';
    }
}
